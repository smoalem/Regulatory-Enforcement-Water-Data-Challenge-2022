...
def linear_regression(X, y):
    ...
    grid_search = GridSearchCV(
        estimator=regressor, param_grid=parameters, scoring="r2", cv=10, n_jobs=-1
    )  # n_jobs = -1 means it will use all CPU cores
...
def logarithmic_regression(X, y):
    ...
    grid_search = GridSearchCV(
        estimator=regressor, param_grid=param_grid, scoring="r2", cv=10, n_jobs=-1
    )  # n_jobs = -1 means it will use all CPU cores
    grid_search.fit(X_train, y_train)
    df_gridsearch = pd.DataFrame(grid_search.cv_results_)
    df_gridsearch["params"] = df_gridsearch["params"].astype("str")
...
def polynomial_regression(X, y):
    ...
    for deg in degrees:
        ...
        test_gridsearch["params"] = test_gridsearch["params"] + " + degree=" + str(deg)
...
def support_vector_regression(X, y):
    ...
    regressor = SVR()
    parameters = [
        {"kernel": ["linear"], "C": [0.01, 0.05]},
        {
            "kernel": ["rbf", "sigmoid"],
            "gamma": [0.0001, 0.01, 0.1, 1, 5, 10],
            "C": [0.01, 0.05],
        },
        {
            "kernel": ["poly"],
...
def color_ratio_reg_contam(reg_contam_pop_axred_gwsw):
    reg = reg_contam_pop_axred_gwsw[0]
    contam = reg_contam_pop_axred_gwsw[1]
    pop = reg_contam_pop_axred_gwsw[2]
    ax_or_red = reg_contam_pop_axred_gwsw[3]
    gw_sw = reg_contam_pop_axred_gwsw[4]
    ...
    query = f"""SELECT * FROM user_effective_timeline_w_contam_fac WHERE regulation= "{reg}" AND contam_id={contam} AND pop_above_or_below_3300= "{pop}" AND is_ax_or_red_older= "{ax_or_red}" AND gw_or_sw= "{gw_sw}" """
...
if __name__ == "__main__":
    ...
    #         for p in pop_list:
    #             for ra in red_ax_list:
    #                 for w in gw_sw_list:
    #                     reg_contam_list.append([r, c, p, ra, w])
    ...
    conn = wdc_lib.sql_query_conn()
    color_ratio_list = pd.read_sql(
        "SELECT * FROM color_ratio_data", conn
    ).values.tolist()
    ...
    for color in color_ratio_list:
        ...
        contaminant = contam_info_df[contam_info_df["id"] == color[3]][
            [
                "reg_xldate",
                "contam_group",
                "method",
                ...
                "min_xldate",
                "max_xldate",
                "unique_sampled_and_reviewed_facilities",
            ]
        ].values.tolist()
        ...
        del contaminant[0][3]
        color_w_complexity_contam = color[:7] + contaminant[0] + color[7:]
        color_with_complexity_contam_list.append(color_w_complexity_contam)
        color_w_complexity_contam.insert(
            0, min_samples * years_reviewed
        )  # complexity, regulation, min_samples, years_reviewed, contam_id, pop, red_ax, gw_sw, reg_xldate, contam_group, method, dlr, mcl, min_xldate, max_xldate, unique_sampled_and_reviewed_facilities, green_ratio, red_ratio, yellow_ratio

    reg_contam_df = pd.DataFrame(
        color_with_complexity_contam_list,
        columns=[
            "complexity",
            "regulation",
    ...
    # X = reg_contam_df.iloc[
    #     :, [1, 8]
    # ].values  # reg, dlr - log               False           False        7.283e-01
    # X = reg_contam_df.iloc[
    #     :, [1, 8, 9]
    # ].values  # reg, dlr mcl -  log               False           False        7.275e-01
    # X = reg_contam_df.iloc[
    #     :, [1, 6]
    # ].values  # regulation and contam group - log - .7241
    # X = reg_contam_df.iloc[:, [1, 5]].values  # regulation and reg xladate - log - .7136
    # X = reg_contam_df.iloc[
    #     :, [0, 1, 5, 6, 8, 9]
    # ].values  # complexity, reg, reg_xldate, contam group, dlr, mcl - log  w/ fit intcpt True        7.287e-01

    # make list of parameters to include with their index
    # find index of each parameter using a dictionary
    # if parameter needs column transform, find where it's index would be on the list
    # implement statsmodel method for beta regression
    ...
    params_list = {
        ...
        "min_xldate": 13,
        "max_xldate": 14,
        "unique_sampled_and_reviewed_facilities": 15,
    }
    ...
    param_combos_list = [
        ...
        ["complexity", "pop", "red_ax", "gw_sw", "reg_xldate", "contam_group"],
    ]

    for pc in param_combos_list:
        pc_indices = []
        ct_indices = []
        for p in pc:
            pc_indices.append(params_list[p])
            if p in one_hot_list:
                ct_indices.append(pc.index(p))
        print([pc, pc_indices, ct_indices])
        X = reg_contam_df.iloc[:, pc_indices].values
        y = reg_contam_df.iloc[:, -3].values

        ct = ColumnTransformer(
            transformers=[("encoder", OneHotEncoder(sparse=False), ct_indices)],
        ...
        with pd.option_context(
            ...
            print(
                df_all_gridsearch.sort_values(by="mean_test_score", ascending=False)
                .head(10)[["regression", "mean_test_score"]]
                .to_string()
            )
