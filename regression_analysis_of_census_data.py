...
print(df_wsp_score_census.columns.to_list())
...
df_wsp_score_census = df_wsp_score_census[['n_race', 'n_white_alone', 'n_black_alone', 'n_ai_and_an_alone', 'n_asian_alone', 'n_nh_and_opi_alone', 'n_other_alone', 'n_two_or_more_races', 
                                           ...
                                           'number_gw', 'number_sw', 
                                           'basename', 'centlat', 'centlon', 'funcstat', 'geoid', 'geo_id', 'hu100', 'intptlat', 'intptlon', 'lsadc', 'mtfcc', 'name', 'objectid', 'oid', 'population', 'regulating', 'sabl_pwsid', 'state_clas', 'county', 'proportion', 'state', 'tract', 'arealand', 'areawater', 'water_system_number', 
...
overage_above_zero = 0
for i,j in df_wsp_score_census.iterrows():
    if float(j['ave_overage_rate']) > 0:
        overage_above_zero += 1
        # print(j['ave_overage_rate'])

print(df_wsp_score_census)
print(overage_above_zero)
raise ValueError

# !!! Need to fix hh_type and hh_own/hh_rent in census_tract_overlap, looks like they're missing question 1E which is necessary for calc of fraction
...
dataset_columns = ['fract_white_alone','fract_black_alone','fract_ai_and_an_alone', 'fract_asian_alone', 'fract_nh_and_opi_alone', 'fract_other_alone', 'fract_two_or_more_races',
                   ...
                   'num_gw', 'num_sw', 
                   'ws_id', 'water_system_number','water_system_name','ave_red_lean_score', 'ave_score_red_lean_percentile', 'ave_overage_rate', 'overage_percentile']

data_array = []
for i,j in df_wsp_score_census.iterrows():
    max_answers = max(j['n_race'], j['hh_size'], j['n_bachelors_deg'], j['n_hh_income'], j['n_rent_as_pct'], j['n_insurance'])
    ...
    ident_score = [j['ws_id'], j['water_system_number'], j['water_system_name']]
...
# imputer.fit(X[:, 1:3]) # Call fit method of imputer to create averages for all columns. Arguments specify all rows and the Age + Salary columns (ignore country column to avoid error since country is all strings)
