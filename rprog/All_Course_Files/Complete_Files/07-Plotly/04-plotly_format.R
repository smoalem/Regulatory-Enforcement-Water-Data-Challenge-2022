...
foo2 <- rbind( 
  foo %>% filter(forecast==0) %>%  filter(date == max(date)),
  foo %>% filter(forecast==1)
)
...
plt <- plot_ly (data = foo %>% filter(forecast == 0), x=~date, color =~country_name, text=~country_name)
plt <- plt %>% add_trace(y=~metric, type='scatter', mode='lines+markers', marker = list(size = 10), 
                         hovertemplate = paste(
                           paste0('<extra></extra>Country Name: %{text}\n',the_metric,': %{y}\nDate: %{x} ')))
plt <- plt %>% add_trace(data = foo2 , y=~metric, x=~date, color = ~country_name,
